...
form Extract Formant data from labelled points
  sentence Directory_name: CHANGEME/data/segments/all/vowel_segments/
  sentence Log_directory_name: CHANGEME/log/
  sentence Interval_label
  sentence Log_file vowel_formants
...
Create Strings as file list... list 'directory_name$'/*.Sound
...
for ifile to num
  ...
  fileappend 'log_directory_name$''log_file$'.log label'tab$'

  for i to numintervals
    fileappend 'log_directory_name$''log_file$'.log 'i'F1'tab$''i'F2'tab$''i'F3'tab$'
  endfor
  fileappend 'log_directory_name$''log_file$'.log 'newline$'

  for i to num_labels
    select 'textGridID'
    label$ = Get label of interval... labeled_tier_number i
      if label$ = interval_label$
        fileappend 'log_directory_name$''log_file$'.log 'fileName$''tab$'
        ...
        for j to numintervals
          ...
          f3_b = f3+(f3/10)
            if j = numintervals
            fileappend 'log_directory_name$''log_file$'.log
